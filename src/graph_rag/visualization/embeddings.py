...
def load_sample_embeddings(
    ...
    if index.graph_embeddings is None:
        raise ValueError(
            "No graph embeddings in the index; run `python -m graph_rag.pipelines.train_gnn`."
        )
...
def create_comparison_plot(
    ...
    # Add overall title
    fig.suptitle(
        "BERT vs Graph Embeddings Comparison (t-SNE Projection)",
        fontsize=18,
        fontweight="bold",
        y=0.98,
    )
...
def create_overlay_plot(
    ...
    ax.set_title(
        "BERT vs Graph Embeddings Overlay (t-SNE Projection)", fontsize=16, fontweight="bold"
    )
...
def main(sample_size: int = 100, perplexity: int = 30):
    ...
    try:
        ...
        comparison_path = os.path.join(
            settings.output_dir, "bert_vs_graph_embeddings_comparison.png"
        )
